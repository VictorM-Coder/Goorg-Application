...
export function WorkspaceCard({ id, name, description, countActivities, color }: WorkspaceCardProps) {
   const urlCurrent = window.location.href.split("/").find(item => ['inicio', 'workspaces'].includes(item));
   ...
   return (
      ...
      >
         ...
         <div>
            <Link to={`/${urlCurrent}/workspace/${id}`} className="flex-none hover:scale-105 transition-all">
         ...
         <div 
            className="flex items-end flex-1 gap-1 mt-4">
            <Stack size={18} />
